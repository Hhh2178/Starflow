...
export default router.post(
  ...
  async (req, res) => {
    ...
    const [item] = await enqueueVideoJobs(getAuthUser(req), {
      ...
      tracks: [{
        ...
        references: req.body.uploadData.map((reference: any) => ({
          kind: reference.sources === "assets" ? "asset" as const : "storyboard" as const,
          id: reference.id,
        })),
      }],
    }, requestId);
    return res.status(200).send(success({ ...item, message: "已加入视频生成队列" }));
  },
);
